...
async function processPlayer(folderName) {
  try {
    ...
    const encodedFolderName = folderName.replace(/\s/g, '%20');
    ...
    for (const row of gameweekData) {
      ...
      if (gameweekMap.has(round)) {
        console.log(`   ⚠️ Skipping duplicate gameweek ${round}`);
        continue;
      }
    ...
    console.log(`   ✅ Validated ${validGameweeks.length} gameweeks: ${validGameweeks.join(', ')}`);
